...
interface AnalisaPostResponse {
    message: string;
}

export function getAnalisaAll(nip: string, fetchTrigger: boolean = false) {

    const [Loading, setLoading] = useState<boolean>(false);
    const [Error, setError] = useState<boolean>(false);
    const [Analisa, setAnalisa] = useState<DataValue[]>([]);
    ...
    useEffect(() => {
        const getListData = async () => {
            ...
            try {
                setLoading(true);
                const response = await fetch(`${API_URL}/analisa/get-all-data/${nip}/${tahun}`);
                const data = await response.json();
                if (!response.ok) {
                    console.log("error dengan response !ok");
                    setAnalisa([]);
                } else {
                    setAnalisa(data.data);
    ...
    return (
        { Analisa, Loading, Error }
    )
}

export function postAnalisa<TRequest = AnalisaFormValue, TResponse = AnalisaPostResponse>(
